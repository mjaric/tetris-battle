...
export function calculateCellSize(playerCount: number, viewportWidth: number, viewportHeight: number): number {
  ...
  const widthBased = Math.floor((perBoard - GARBAGE_METER_EXTRA - BORDER_EXTRA) / BOARD_COLUMNS);
  const heightBased = Math.floor((viewportHeight - VERTICAL_OVERHEAD) / BOARD_ROWS);
